...
3. Combine #2 and #3 to **generate a DGElist** using `DGElist(counts=<data>, group=<grouping>)`
4. **Filter** genes with low counts using `data[keep]`
5. **Normalize** the counts post-filtering via `calcNormFactors()`
6. Convert #3 into an **experimental design** using `model.matrix()` for linear modeling. 
